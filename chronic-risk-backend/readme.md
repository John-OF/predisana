...
- **Un solo split por enfermedad (AUD-15).** `train_nhanes_diabetes.py` hacía su propio `train_test_split` del CSV completo mientras `data_curated/diabetes/*` salía de `curate_and_synthesize.py`. Para la variante **con glucosa** los dos repartos no coincidían —filtraba las filas sin glucosa *antes* de partir, así que `n` cambiaba—, con dos consecuencias: el AUC publicado en `/metricas` no se medía sobre el test que sirve el laboratorio, y el **fondo de SHAP** (que se carga de `diabetes_train.csv`) contenía el **79%** de las filas de test de esa variante. Ahora ambos scripts usan `load_split_or_fallback()`. La variante self-report no estaba afectada: su reparto coincidía por casualidad con el curado (mismas filas, misma semilla).
- **Aviso de cobertura de datos (AUD-16).** Un modelo no avisa de que está extrapolando: devuelve un número igual de firme para una edad que vio 5000 veces que para una que no vio nunca (cardiovascular se entrenó con **29,7-64,9 años** y el formulario acepta 18-100). Al arrancar se calcula `SUPPORT` desde el train curado —el mismo CSV que alimenta el fondo de SHAP— y `/predict` devuelve `support_warnings` (`sin_datos` fuera del min-max observado, `pocos_datos` fuera del p1-p99) y `support_note`. Igual que la capa clínica, va **desacoplado**: no toca la probabilidad. `/config` expone `feature_support` y `/whatif` el `supported_range` de la variable barrida, que el frontend sombrea en la curva — ahí se aplana por falta de datos, no porque el riesgo deje de subir.
- **El entrenamiento del GAN se mide en pasos, no en épocas (AUD-24).** CTGAN hace `ceil(n_filas / 500)` actualizaciones por época, así que un `--epochs` fijo daba **10x menos entrenamiento** a un dataset de 4,8k filas que a uno de 55k. Se notaba: a 50 épocas la correlación peso↔cintura del sintético de hipertensión era **−0,08** frente a **+0,90** en el real, y el **22%** de las filas eran físicamente imposibles (peso <75 kg con cintura >115 cm). El parámetro real es **`--steps`** (default 15000) y las épocas se derivan por dataset; `--epochs` sigue estando para forzarlas a mano. 15000 es donde satura: la correlación peso↔cintura va −0,08 (500 pasos) → +0,55 (5k) → +0,70 (15k) → +0,72 (30k), y el **ruido entre ejecuciones con la misma configuración es de ese tamaño** (repetir los 15k dio +0,74). No merece la pena afinar sobre diferencias <0,05. Aparte, las marginales categóricas se desviaban entre **0,05 y 0,18 en cualquier número de pasos** (el colesterol alto de hipertensión salía entre 46% y 58% frente al 40% real) y eso no lo arreglaba entrenar más ni menos: se corrige **después de muestrear**, generando un pool mayor (`--oversample`, 4 por defecto) y submuestreándolo para que la distribución **conjunta** de lo categórico coincida con la real (`--no_match_marginals` lo desactiva). Los cupos se reparten por **resto mayor**: con un redondeo normal los estratos pequeños caen a cero y los grandes absorben su masa, un sesgo peor que el problema original. **Importante para contarlo bien:** así las marginales categóricas quedan *impuestas*, no aprendidas — la mejora de SDMetrics en esas columnas es por construcción; la parte continua sigue siendo del GAN. El ajuste solo **elige** filas: nunca las inventa ni las repondera. `PASOS_POR_DATASET` deja cardiovascular en 5500, donde ya estaba, porque su única corrida a 15k puntuó peor (0,924 → 0,893) y no hay razón para sustituir el artefacto que mejor puntúa. Entrenar más también acerca al GAN a memorizar filas reales, así que se comprobó: la distancia mediana de un sintético a su vecino real más cercano se queda en **1,2-1,8x** la del propio test real, y las colisiones exactas (3 de 54392 en cardiovascular, donde el dato es grueso) son **150x más raras** que entre el train y el test reales.
...
- **Coherencia peso/IMC/cintura en hipertensión (auditoría externa, 2026-08).** Hipertensión pide `weight`, `bmi` y `waist_circumference` como 3 campos sueltos, sin altura ni validación cruzada entre ellos. En los datos reales están fuertemente correlacionados (r≈0,89-0,90); el LogReg ganador del bake-off aprendió, por colinealidad, un coeficiente **negativo** para `weight` (−0,37 — el único signo invertido entre las features con dirección clínica inequívoca: age +0,84, bmi +0,56, waist_circumference +0,29, high_cholesterol +0,37, diabetes +0,26, heart_disease +0,25). Con datos que covarían de forma realista el modelo predice bien (a más tamaño corporal, más riesgo); el síntoma solo aparece con combinaciones incoherentes (p.ej. cintura de 115 cm con IMC 19, o 180 kg con IMC 25), donde el riesgo puede **bajar** al subir el peso — y antes ningún aviso lo detectaba, porque `compute_support_warnings` mira una feature a la vez y cada campo, por separado, cae dentro de su rango individual. `_check_coherencia_corporal()` agrega un tercer chequeo: una altura implícita `sqrt(weight/bmi)` fuera de [1,30, 2,20] m, o un `bmi`/`waist_circumference` en combinación fisiológicamente casi imposible (IMC≤22 con cintura≥100cm, o IMC≥35 con cintura≤80cm). Igual que el resto de esta capa, **no toca la probabilidad**: agrega un nivel `incoherente` más a `support_warnings`. Ese nivel no trae `trained_range` (no hay un rango que citar: son varios campos que no cuadran entre sí), así que el frontend lo pinta aparte con su `detail` en `AvisoSoporte.jsx`; la primera versión asumía `trained_range` en todos los avisos y con un `incoherente` la página del simulador se rompía entera.
- **Presión sistólica/diastólica invertida en el sintético de cardiovascular (auditoría externa, 2026-08, sin corregir).** El 0,13% de las filas sintéticas de cardiovascular (73 de 54.392) tiene `ap_hi < ap_lo` (sistólica menor que diastólica), fisiológicamente imposible; en el real es 0%. A diferencia del caso peso↔cintura de hipertensión (AUD-24), esto no se auditó ni se corrigió en `curate_and_synthesize.py`: no hay ninguna constraint tipo `ap_hi >= ap_lo` sobre el CTGAN. `/predict` tampoco avisa de esta combinación si se le manda directamente (no existe, a diferencia del punto anterior, un chequeo de coherencia entre `ap_hi` y `ap_lo`).
